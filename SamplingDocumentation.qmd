---
title: "COMM Consult validation data"
subtitle: "Sampling texts for human coding"
author:
  - name: "Christian Rauh"
    url: "http://www.christian-rauh.eu"
date: today
format:
  html:
    toc: true
    ...
    code-fold: true            # add show/hide toggles above each code block
    code-summary: "Show R code"  # text on the toggle button
    code-tools: true           # adds copy/download buttons
    code-copy: true
    embed-resources: true      # <-- makes a single HTML file (images/CSS/JS inlined)
    link-external-newwindow: true   # open external links in new tab
    include-in-header:
      text: |
        <style>
        /* Justify narrative text */
        #quarto-content p,
        #quarto-content li,
        ...
        /* Keep headings and code non-justified */
        #quarto-content h1, #quarto-content h2, #quarto-content h3,
        #quarto-content h4, #quarto-content h5, #quarto-content h6 {
          text-align: left;
        }
        #quarto-content pre,
        #quarto-content code,
        #quarto-content .sourceCode,
        #quarto-content .cell-output pre {
...
We need to validate our text-based measures for 'information quality' of consultation responses against the assessments of human coders.

Coding full texts is not possible - text length varies drastically and can span several pages of text. We thus agreed to let them code **shorter units (3-4 sentences)**. As we have no consistent formatting markers in the text data (such as paragraphs or line breaks), we decided to take the first few sentences of each contribution (typically containing the key messages) of individual consultation responses. In this context, we want to exclude the first sentence, as this often just informs about who the commenter is.
...
We want to assess intercoder reliability as well, requiring us to generate an overlap sample seen by all coders, meaning that the targeted samples are the following:

* One 'overlap' sample of 100 text snippets (contained in all sets)   
* Three separate/distinct samples of 500 text snippets (one for each coder)  

For each coder, these samples should cover the full breadth/variation of the text measures we want to validate, while also keeping the policy stage of the respective consultation balanced as this is the main analytic distinction in the manuscript.

Accordingly, my work plan is as follows:

1. Load full text data and extract text snippets
2. (Re-) Calculate text measures for the snippets
3. Draw the four stratified random samples (avoid duplicates across samples to maximize number of obs)
4. Build and store three samples to be fed into the coding app (unique id, keep measured values, mark overlap obs)

All code is also available on **[GitHub](https://github.com/ChRauh/CONSULT-ValidationApps)**.

The R packages used for this exercise:

```{r}
# Packages used #####
library(tidyverse) # Easily Install and Load the 'Tidyverse' CRAN v2.0.0
library(purrr) # Functional Programming Tools CRAN v1.0.4
library(tokenizers) # Fast, Consistent Tokenization of Natural Language Text CRAN v0.3.0
library(quanteda) # Quantitative Analysis of Textual Data CRAN v4.3.1
library(cld2) # Google's Compact Language Detector 2 CRAN v1.2.6
library(textclean) # Text Cleaning Tools CRAN v0.9.3
...
* I use the latest data file from Dropbox
* Dropping all observations with empty *\`text_full\`* observations
...
I took a **couple of choices here**:

* Remove the first sentence of the comment - but only if overall comment is more than two sentences long
* We want the human coders to see meaningful text (= full sentences), while avoiding overly long comments and preventing too much variation in the length of the snippets coders get to see: I selected **anything up to the sentence that contains the 200th word**.   
...
# But we also have a number of shorter feedback which we do not want to artificially reduce too much
# Thus we remove the first sentence only when the overall comment is more than 2 sentences long
...
# Helper function
# We want the human coders to see meaningful text (= full sentences), while avoid overly long comments 
...
Have a look at how long (number of words) the extracted snippets are:

```{r}
#| eval: true  
#| message: false  # optional: silence messages
#| warning: false  # optional: silence warnings
library(tidyverse)
read_rds("./data/snippets.rds") %>% 
  ggplot(aes(x = snippet_wc))+
  geom_density()+
  labs(title = "Snippet lengths: Word count of text snippts across the full data set")+
  theme_bw()

```

By construction, they cluster around the cutoff criterion of 200 words (or higher, for commenters using long sentences). But we also have a couple of (much) shorter ones in cases when commenters actually supplied (significantly) less than 200 words.

While ultra-short observations should be probably excluded from human coding (see below), I think **we should allow some variation in this regard** (as these are real cases). The alternative would be to select only cases that have at least 200 words, then the coder samples would be more balanced regarding length.

<br>

# Calculate 'information quality' for snippets

Then I extracted all the text-based measures we use in the full text analysis also for the short snippets (following the original code ). These data allow us later to offer the coders with texts that also vary sufficiently across the measures we want to validate.
...
Let's quickly compare the distribution of our four information quality measures in the snippets to that observed in the full feedback texts:  
...
```
![](plots/TextMeasuresFullVsSnippets.png)

Looks largely reasonable, with a few notable points:

* **Entropy** differs strongly and is more constrained for the shorter snippets - somehow logical as shorter text limit the diversity of the vocabularly one can use. Still sufficient variation in the snippets for validation, but one should think about normalization in the main analysis (as we probably capture text length implicitly).

* Fewer **entities** in the snippets - also reasonable with regard to text length and considering that we only look at the first few sentences. But also sufficient variation on offer for validation.

* **Flesch** readability measure not capped as in the main analysis, done for the sampe below.
<br>
<br>

# Draw coder samples

Clean up the data on the *population* to draw from a little further along the above observations:

* Keep only snippet based information
* **Remove extremely short observations (< than 10 words)**
* Clean up encoding errors in the raw texts (**should be done in main analyses as well!**)
...
Now for the tricky part - **random stratified samples of snippets such that**: 

* the full variation of our text measures is seen by each coder
* each coder gets a sample balanced as to the policy stage of the consultation (n = 500)
* we have an overlap of n = 100 snippets across all three coders to study human reliability
* avoid any other text duplicates in the data
...
To check whether stratified sampling has worked as planned, we can compare the distribution of our four text measures in the snippet population to that in each of the four samples (sample 1 is the smaller overlap sample for intercoder reliability):
...
![](plots/MeasureDistributionInCoderSamples.png)

The n=100 overlap sample is a bit more off than the three n= 500 larger samples, but that doens't matter for our validation. **All in all, the samples for the human coders reflect the distribution in the full population rather well.**   
<br>
Then check, whether this also holds regarding the demanded **balance across policy stages**:
...
![](plots/PolicyStagesInCoderSamples.png)

Good, almost perfectly balanced in each of the samples (while legislative proposal feedback is somewhat overrepresented in the full feedback data).    
<br>

Qualitative perspective - some **random sample snippets** of the type the coders will see:
...
Now I combine each of the n=500 samples with the n=100 overlap sample (marked in the data), shuffle the rows in each, and export them as csv files that are later used as input for the coding app.
...
# Notes for the main analyses

* Many texts contain list items or annex tables that set off our text based measures (incorrect sentence tokenization). For list items that can be corrected, by replacing respective patterns (e.g. "; - ([a-z])") by sentence-end punctuation.

* The *\`text_full\`* variable contains many encoding errors which should be corrected before measurement (e.g. by *\`mutate(text = iconv(text, from = "latin1", to = "UTF-8"), text = textclean::replace_non_ascii(text))\`*).

* Think about entropy normalisation
